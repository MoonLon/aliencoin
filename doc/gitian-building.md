...
*Setup instructions for a gitian build of Aliencoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Aliencoin
...
- [Building Aliencoin](#building-aliencoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-aliencoin-host/README.md).
...
Clone the git repositories for aliencoin and gitian and then checkout the aliencoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/aliencoin/aliencoin
cd aliencoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the aliencoin repository
...
To build Aliencoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the aliencoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/aliencoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/aliencoin/aliencoin
...
For example:
```bash
URL=https://github.com/laanwj/aliencoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit aliencoin=${COMMIT} --url aliencoin=${URL} ../aliencoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit aliencoin=${COMMIT} --url aliencoin=${URL} ../aliencoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit aliencoin=${COMMIT} --url aliencoin=${URL} ../aliencoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/aliencoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/aliencoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/aliencoin-build.assert
```
...
[aliencoin/gitian.sigs](https://github.com/aliencoin/gitian.sigs/) repository, or if that's not possible create a pull
