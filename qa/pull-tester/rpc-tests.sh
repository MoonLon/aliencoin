...
# Get BUILDDIR and REAL_ALIENCOIND
. "${CURDIR}/tests-config.sh"

export ALIENCOINCLI=${BUILDDIR}/qa/pull-tester/run-aliencoin-cli
export ALIENCOIND=${REAL_ALIENCOIND}
...
if [ "x${ENABLE_ALIENCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and aliencoind must all be enabled"
fi
