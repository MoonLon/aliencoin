...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars ALIENCOIND and ALIENCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

ALIENCOIND=${ALIENCOIND:-${1}/aliencoind}
CLI=${ALIENCOINCLI:-${1}/aliencoin-cli}
...
$ALIENCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$ALIENCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
